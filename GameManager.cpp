...
void GameManager::ShowMainMenu() {
    std::cout << "\nДЕНЬ " << currentDay << " | ЗОЛОТО: " << school.getGold() << " | РЕПУТАЦИЯ: " << school.getReputation()
    << std::endl;
    ...
    if (choice == 1) {
        ...
        if (school.GetGladiatorCount() == 0) std::cout << "\nУ вас нет гладиаторов! Сначала купите раба на Рынке." << std::endl;
            ...
            else std::cout << "Неверный номер бойца!" << std::endl;
        }
    }
    else if (choice == 4) {
        EndDay();
